...
Shader "AP01/L09/OldSchoolPro_SF" {
    Properties {
        ...
        _node_4288 ("node_4288", Cube) = "_Skybox" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform samplerCUBE _node_4288;
...
////// Emissive:
                ...
                float3 node_1116 = mul( _NormalMap_var.rgb, tangentTransform ).xyz;
                ...
                float3 emissive = (((attenuation*(_LightCol_var.rgb*((_BaseCol_var.rgb*max(dot(i.normalDir,lightDirection),0.0))+pow(max(dot(lightDirection,viewReflectDirection),0.0),_SpecPow_var))))+(_AO_var.r*((((_EnvUpCol_var.rgb*node_4693)+(_EnvSideCol_var.rgb*(1.0-(node_4693+node_555))))+(node_555*_EnvDownCol_var.rgb))*_EnvInt_var)))+((texCUBElod(_node_4288,float4(reflect((viewDirection*(-1.0)),node_1116.rgb),_CubemapMip_var)).rgb*pow(1.0-max(0,dot(node_1116.rgb, viewDirection)),_FresnelPow_var))*_envSpecInt_var));
